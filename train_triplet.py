                ...
                with tqdm(enumerate(dataloaders[phase])) as tbatch:
                    for i, batch in tbatch:
                        ...
                        with torch.set_grad_enabled(phase == "train"):
                            optimizer.zero_grad()
                            outputs, anchor, samples = model(sentences_ls, TRAIN_IDX)
                            idx_selection_pos = [
                                (i, l)
                                for i, labels in enumerate(labels_ls)
                                for l in labels
                                for _ in range(50)
                            ]
                            ...
                            idx_selection_neg = [
                                ...
                                for r in np.random.choice(
                                    [emo for emo in TRAIN_IDX if emo not in labels],
                                    50,
                                )
                            ]
                            # idx_selection_neg = [
                            ...
                            #     for i, labels in enumerate(labels_ls)
                            #     for _ in labels
                            #     for r in [emo for emo in TRAIN_IDX if emo not in labels]
                            # ]

                            anchor = torch.stack(
                                [anchor[idx] for idx in idx_selection_pos]
                            )
                            pos = torch.stack(
                                [samples[idx] for idx in idx_selection_pos]
                            )
                            neg = torch.stack(
                                [samples[idx] for idx in idx_selection_neg]
                            )
...
if __name__ == "__main__":
    ...
    # criterion = nn.TripletMarginLoss(margin=500)
